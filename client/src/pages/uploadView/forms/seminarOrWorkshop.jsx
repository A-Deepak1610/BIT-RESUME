...
// --- Radio Group Component (Unchanged) ---
...
id, name, label, onFileSelect, selectedFile, error, helperText, required, setFormError, className = ""
    ...
    const processFile = (file) => {
        ...
        if (file) {
            ...
            let isValidType = acceptedTypes.includes(fileExtension) || acceptedTypes.includes(fileMimeType) || (fileMimeType === 'application/pdf' && acceptedTypes.includes('.pdf')) || (fileMimeType === 'image/png' && (acceptedTypes.includes('image/png') || acceptedTypes.includes('.png'))) || (fileMimeType === 'image/jpeg' && (acceptedTypes.includes('image/jpeg') || acceptedTypes.includes('.jpg') || acceptedTypes.includes('.jpeg')));
            if (!isValidType) {
    ...
    return (
        <div className={`mb-6 ${className}`}>
            ...
            >
                ...
                <input
                    id={id} name={name} type="file" className="sr-only"
                    onChange={handleFileChange} accept={ACCEPT_STRING} ref={fileInputRef}
                />
            </div>
            {selectedFile && !error && (
...
// --- Options and Validation (Unchanged) ---
...
const EventStep1_BasicDetails = ({ formData, handleChange, errors }) => (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <InputField id="eventTitle" name="eventTitle" label="Title of the Event" value={formData.eventTitle} onChange={handleChange} error={errors.eventTitle} placeholder="Enter event title" required className="md:col-span-2" />
          <SelectField id="eventType" name="eventType" label="Type of Event" value={formData.eventType} onChange={handleChange} options={eventTypeOptions} placeholder="Select event type" error={errors.eventType} required />
          <SelectField id="deliveryMode" name="deliveryMode" label="Mode of Delivery" value={formData.deliveryMode} onChange={handleChange} options={deliveryModeOptions} placeholder="Select delivery mode" error={errors.deliveryMode} required />
          <SelectField id="eventNature" name="eventNature" label="Event Nature" value={formData.eventNature} onChange={handleChange} options={eventNatureOptions} placeholder="Select event nature" error={errors.eventNature} required />
          <InputField id="organizedBy" name="organizedBy" label="Organized By" value={formData.organizedBy} onChange={handleChange} error={errors.organizedBy} placeholder="Organization name" required />
          <InputField id="location" name="location" label="Event Location / Platform" value={formData.location} onChange={handleChange} error={errors.location} placeholder="e.g., Grand Hall, New York / Online via Zoom" helperText="Specify the city, venue, or the online platform where the event was held." required className="md:col-span-2" />
      </div>
    </div>
);
...
const EventStep2_ScheduleParticipation = ({ formData, handleChange, errors }) => (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          <InputField id="eventStartDate" name="eventStartDate" label="Event Start Date" type="date" value={formData.eventStartDate} onChange={handleChange} error={errors.eventStartDate} required />
          <InputField id="eventEndDate" name="eventEndDate" label="Event End Date" type="date" value={formData.eventEndDate} onChange={handleChange} error={errors.eventEndDate} required />
      </div>
      <div className="pt-4">
          <h3 className="text-lg font-medium text-gray-700 mb-3 flex items-center">
              <Users className="h-5 w-5 text-indigo-500 mr-2" /> Participation Type
          </h3>
          <RadioGroupField name="participationRole" label="Participation Role" value={formData.participationRole} onChange={handleChange} options={participationRoleOptions} error={errors.participationRole} required className="md:col-span-2" />
      </div>
    </div>
);
      ...
      <div className="grid grid-cols-1 gap-y-0">
          <div className="mb-4">
              <label className="flex items-center space-x-3 cursor-pointer">
                  <input type="checkbox" name="isCertificateProvided" checked={formData.isCertificateProvided} onChange={handleChange} className="focus:ring-indigo-500 h-4 w-4 text-indigo-600 border-gray-300 rounded" />
          ...
          {formData.isCertificateProvided && (
              <FileUploadField id="certificate" name="certificate" label="Upload Certificate" selectedFile={formData.certificate} onFileSelect={handleFileSelect} error={errors.certificate} required={formData.isCertificateProvided} setFormError={setFormError} helperText={SUPPORTED_FORMATS_LABEL} />
          )}
          <InputField id="eventLink" name="eventLink" label="Link to Event Page / Recording / YouTube" value={formData.eventLink} onChange={handleChange} error={errors.eventLink} placeholder="e.g., https://example.com/event or https://youtu.be/videoID" helperText="A valid link to the event page or recording is required (e.g., https://example.com)" required />
      </div>
    </div>
);
...
const EventStep4_LearningRelevance = ({ formData, handleChange, errors }) => (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 gap-4 md:gap-6">
          <TextareaField id="topicsCovered" name="topicsCovered" label="Topics Covered" value={formData.topicsCovered} onChange={handleChange} error={errors.topicsCovered} placeholder="List key topics covered, separated by commas (e.g., AI, CAD, Project Management)" rows={3} className="md:col-span-2" required />
          <TextareaField id="skillsGained" name="skillsGained" label="Skills or Knowledge Gained" value={formData.skillsGained} onChange={handleChange} error={errors.skillsGained} placeholder="Describe skills or knowledge you gained from this event" rows={4} className="md:col-span-2" required />
          <TextareaField id="relevance" name="relevance" label="Relevance to Academic or Career Path" value={formData.relevance} onChange={handleChange} error={errors.relevance} placeholder="How is this event relevant to your academic or career goals?" rows={4} className="md:col-span-2" required />
      </div>
    </div>
);
...
// --- Main SeminarOrWorkshop Component (UPDATED) ---
...
const SeminarOrWorkshop = ({ onBack, initialData = {} }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (DEBUG_MODE) {
            console.log(`[DEBUG] Submitting event for rollno: ${rollno}`);
        }
        ...
        payload.append('is_certificate_provided', formData.isCertificateProvided.toString());
        ...
        <div className="max-w-3xl mx-auto bg-white py-4 md:py-8 px-4 sm:px-6 lg:px-10 mt-2 md:mt-4 rounded-lg shadow-xl">
            ...
            <div className="mb-6 px-2 pt-2">
                <div className="md:hidden mb-4">
                    <h2 className="text-lg font-semibold text-gray-800">{STEP_CONFIG_EVENT[currentStep].title}</h2>
                </div>
                            ...
                            <li key={step.title} className={`relative ${stepIdx !== STEP_CONFIG_EVENT.length - 1 ? 'pr-8 sm:pr-10 md:pr-12' : ''} flex-1`}>
                                {stepIdx < currentStep ? (
                                    <>
                                        ...
                                        <button type="button" onClick={() => { setCurrentStep(stepIdx); setErrors({}); }} disabled={isSubmitting} className="relative w-10 h-10 flex items-center justify-center bg-indigo-600 rounded-full hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-70">
            ...
            <div className="bg-white py-6">
                <form noValidate>
                    <div className="min-h-[400px] mb-8 px-1.5">{renderStepContent()}
                        {errors.submit && (<p className="mt-6 text-center text-sm text-red-600 bg-red-50 p-3 rounded-md">{errors.submit}</p>)}
                    </div>
                    <div className="mt-10 pt-6 flex justify-between items-center border-t border-gray-200 px-1.5">
                        {currentStep > 0 ? (
                            <button type="button" onClick={prevStep} disabled={isSubmitting} className="px-7 py-2.5 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 cursor-pointer disabled:opacity-50">Back</button>
                        ) : (<div />)}
                        {currentStep < STEP_CONFIG_EVENT.length - 1 ? (
                            <button type="button" onClick={nextStep} disabled={isSubmitting} className="inline-flex items-center justify-center px-7 py-2.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer disabled:opacity-50">Next</button>
                        ) : (
                            <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="inline-flex items-center justify-center px-7 py-2.5 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 cursor-pointer disabled:opacity-50 disabled:bg-indigo-400">
