...
import {
  ...
  Lightbulb,
  Info,
  Upload, // Standard Icons
  Briefcase,
  CalendarDays,
  Sparkles, // Internship Specific Icons (LinkIcon removed as githubLink removed)
  Award,
  MessageSquare,
  Target,
  AlertTriangle,
    ...
    if (file) {
      ...
      if (fileSizeMB > MAX_FILE_SIZE_MB) {
        if (setFormError)
          ...
          console.error(`File exceeds maximum size of ${MAX_FILE_SIZE_MB}MB.`);
      ...
      {selectedFile && !error && (
        <div className="mt-2 flex items-center justify-between text-sm text-gray-700 bg-gray-50 p-2 rounded-md border border-gray-200">
          <span className="truncate flex items-center">
            <FileText
              size={16}
              className="inline mr-2 text-gray-500 flex-shrink-0"
            />
...
const validateStep1_Internship = (formData) => {
  ...
  if (!formData.company_name?.trim())
    errors.company_name = "Organization/Company name is required.";
  if (!formData.roll?.trim())
    errors.roll = "Internship role/title is required.";
  if (!formData.domain?.trim())
    errors.domain = "Internship domain is required.";
  if (!formData.internship_type)
    errors.internship_type = "Internship type is required.";
...
const validateStep2_Internship = (formData) => {
  const errors = {};
  if (
    formData.industry_mentor_contact &&
    !/^\S+@\S+\.\S+$/.test(formData.industry_mentor_contact)
  ) {
    errors.industry_mentor_contact =
      "Please enter a valid email address for the industry mentor.";
  }
  return errors;
};
...
const validateStep3_Internship = (formData) => {
  ...
  if (!formData.offer_letter)
    errors.offer_letter = "Offer letter (PDF, PNG, JPG) is required.";
  if (!formData.report)
    errors.report = "Internship report (PDF, PNG, JPG) is required.";
  return errors;
};

const validateStep4_Internship = (formData) => {
  const errors = {};
  if (!formData.skill_gained?.trim())
    errors.skill_gained = "Skills gained is required.";
  if (!formData.outcomes?.trim())
    errors.outcomes = "Key outcomes or achievements are required.";
  return errors;
};
...
const InternshipStep1_CoreDetails = ({ formData, handleChange, errors }) => (
  <div className="space-y-6">
    ...
    <CheckboxField
      id="is_stipend"
      name="is_stipend"
      label="This was a paid internship (received stipend)"
      checked={formData.is_stipend}
      onChange={handleChange}
      description="Check this if you received a stipend"
    />
  </div>
);
...
const InternshipStep2_Supervision = ({ formData, handleChange, errors }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
      <InputField
        id="consulted_faculty_name"
        name="consulted_faculty_name"
        label="Consulted Faculty Name (Optional)"
        value={formData.consulted_faculty_name}
        onChange={handleChange}
        error={errors.consulted_faculty_name}
        placeholder="e.g. Dr. Smith"
      />
      <InputField
        id="industry_mentor_name"
        name="industry_mentor_name"
        label="Industry Mentor Name (Optional)"
        value={formData.industry_mentor_name}
        onChange={handleChange}
        error={errors.industry_mentor_name}
        placeholder="e.g. John Doe"
      />
    </div>
    <InputField
      id="industry_mentor_contact"
      name="industry_mentor_contact"
      label="Industry Mentor Contact (Optional)"
      value={formData.industry_mentor_contact}
      onChange={handleChange}
      error={errors.industry_mentor_contact}
      placeholder="e.g. john@company.com"
      type="email"
      helperText="Email or phone number of industry mentor"
    />
  </div>
);

const InternshipStep3_Documentation = ({
  formData,
  handleFileSelect,
  errors,
  setFormError,
  ...
  <div className="space-y-6">
    ...
    <FileUploadField
      id="report"
      name="report"
      label="Upload Internship Report"
      selectedFile={formData.report}
      onFileSelect={handleFileSelect}
      error={errors.report}
      helperText="PDF, PNG, or JPG file of your internship report."
      required
      setFormError={setFormError}
      py="py-6"
    />
    ...
    <div className="space-y-8">
      ...
      <section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
          ...
          <DetailItem
            label="Received Stipend"
            value={formData.is_stipend}
            isBoolean
          />
        </div>
      </section>
...
const STEP_CONFIG_INTERNSHIP = [
  {
    title: "Core Details",
    validate: validateStep1_Internship,
    icon: Briefcase,
  },
  { title: "Supervision", validate: validateStep2_Internship, icon: Users },
  {
    title: "Documentation",
    validate: validateStep3_Internship,
    icon: FileText,
  },
...
const Internship = ({ onBack, initialData = {} }) => {
  ...
  const validateCurrentStep = useCallback(() => {
    ...
    if (errors.offer_letter && !currentStepErrors.offer_letter)
      preservedFileErrors.offer_letter = errors.offer_letter;
    if (errors.report && !currentStepErrors.report)
      preservedFileErrors.report = errors.report;
  ...
  const nextStep = useCallback(() => {
    if (!validateCurrentStep()) {
      if (DEBUG_MODE)
        console.log("Validation failed for next step. Errors:", errors);
      window.scrollTo(0, 0);
      return;
    }
    setCurrentStep((prev) =>
      Math.min(prev + 1, STEP_CONFIG_INTERNSHIP.length - 1)
    );
    window.scrollTo(0, 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (currentStep !== STEP_CONFIG_INTERNSHIP.length - 1) {
      if (DEBUG_MODE)
        console.log(
          "handleSubmit called, but not on review step. Current step:",
          currentStep
        );
      return;
    }
    ...
    for (let i = 0; i < STEP_CONFIG_INTERNSHIP.length - 1; i++) {
      // Exclude review step
    ...
    if (errors.offer_letter) {
      ...
      if (
        firstErrorStep === -1 ||
        firstErrorStep >
          STEP_CONFIG_INTERNSHIP.findIndex((s) => s.title === "Documentation")
      ) {
        firstErrorStep = STEP_CONFIG_INTERNSHIP.findIndex(
          (s) => s.title === "Documentation"
        );
      }
    }
    if (errors.report) {
      // Though report is optional, if there's an error (e.g. size/type), it's still an error
      allValid = false;
    ...
    if (!allValid) {
      ...
      if (DEBUG_MODE)
        console.log(
          "Full form validation failed on submit:",
    ...
    if (
      formData.consulted_faculty_name &&
      formData.consulted_faculty_name.trim()
    ) {
      payload.append(
    ...
    if (formData.industry_mentor_name && formData.industry_mentor_name.trim()) {
      payload.append(
        ...
        formData.industry_mentor_name.trim()
      );
    }
    if (
      formData.industry_mentor_contact &&
      formData.industry_mentor_contact.trim()
    ) {
      payload.append(
    ...
    try {
      const response = await axios.post(
        "http://localhost:6001/api/internships",
        payload,
        {
      ...
      if (DEBUG_MODE)
        console.log("Internship submission successful:", response.data);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 2:
        return (
          <InternshipStep3_Documentation
            formData={formData}
            handleFileSelect={handleFileSelect}
            errors={errors}
            setFormError={setFormErrorForFile}
          />
        );
      case 3:
        return (
          <InternshipStep4_Reflections
            formData={formData}
            handleChange={handleChange}
            errors={errors}
          />
      ...
      case 4:
        return <InternshipStep5_Review formData={formData} />;
      default:
        return null;
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto bg-white py-10 px-4 sm:px-6 lg:px-8 mt-4 text-center rounded-lg shadow-xl">
        ...
        <h2 className="text-3xl font-bold text-gray-900 mb-3">
          Internship Details Submitted!
        </h2>
        <p className="text-gray-600 mb-8 text-lg">
          Your internship information has been successfully submitted for
          review.
        </p>
        {errors.submit && (
          <p className="my-4 text-md text-red-600 bg-red-50 p-3 rounded-md">
            {errors.submit}
          </p>
        )}
      ...
      <div className="flex items-center mb-8 border-b pb-5 border-gray-200">
        <div className="flex-grow">
          ...
          <p className="mt-1.5 text-sm text-gray-500">
            Fill in the details for your internship experience.
          </p>
        </div>
      </div>
          ...
          <ol role="list" className="flex items-center">
            {STEP_CONFIG_INTERNSHIP.map((step, stepIdx) => (
              <li
                key={step.title}
                className={`relative ${
                  stepIdx !== STEP_CONFIG_INTERNSHIP.length - 1
                ...
                {stepIdx < currentStep ? (
                  <>
                    ...
                    >
                      ...
                      <span className="absolute -bottom-7 text-center w-max max-w-[100px] text-xs font-medium text-indigo-600 truncate">
                        {step.title}
                      </span>
                    </button>
                  </>
                  ...
                  <>
                    ...
                    >
                      ...
                      <span className="absolute -bottom-7 text-center w-max max-w-[100px] text-xs font-bold text-indigo-600 truncate">
                        {step.title}
                      </span>
                    </div>
                  </>
                  ...
                  <>
                    ...
                    <div className="group relative w-10 h-10 flex items-center justify-center bg-white border-2 border-gray-300 rounded-full cursor-not-allowed">
                      ...
                      <span className="absolute -bottom-7 text-center w-max max-w-[100px] text-xs font-medium text-gray-500 truncate">
                        {step.title}
                      </span>
