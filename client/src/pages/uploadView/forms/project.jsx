...
const validateStep5 = (formData) => {
  ...
  // reportPdfFile is optional on the UI, so no validation here unless explicitly made required.
  return errors;
};
...
const Project = ({ onBack, initialData = {} }) => {
    ...
    setFormData(prev => {
        ...
        if (prev.projectType === 'Individual') {
            if (newTeamMembers.length > 1) {
                newTeamMembers = [{ ...newTeamMembers[0] }]; 
  ...
  const handleSubmit = async (event) => {
    ...
    // payload.append('rollno', formData.teamMembers[0]?.rollNumber?.trim() || '');
    ...
    // If report_pdf is not provided, it won't be appended.
    // Ensure backend handles http.ErrMissingFile gracefully if it's truly optional there.
    // If backend *requires* it, frontend validation in validateStep5 should enforce it.
          ...
          <div className="mt-3 pt-6 flex justify-between items-center border-t border-gray-200 px-1.5">
            ...
            {currentStep < STEP_CONFIG.length - 1 ? (
              <button
                type="button"
                onClick={nextStep}
                disabled={isSubmitting} // Keep isSubmitting for Next button too if you want to prevent navigation during submission attempt
