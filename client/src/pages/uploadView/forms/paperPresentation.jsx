...
const validateStep1_PaperPresentation = (formData) => {
  ...
  if (!formData.conference_title?.trim())
    errors.conference_title = "Conference title is required.";
  if (!formData.location?.trim()) errors.location = "Location is required.";
  if (!formData.date_of_presentation)
    errors.date_of_presentation = "Date of presentation is required.";
  return errors;
};
  ...
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
      <InputField
        ...
        placeholder="e.g. Advancements in AI"
        required
      />
      <InputField
        id="conference_title"
        ...
        onChange={handleChange}
        error={errors.conference_title}
        placeholder="e.g. International AI Summit"
        required
      />
    </div>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
      ...
      <InputField
        ...
        type="date"
        value={formData.date_of_presentation}
        onChange={handleChange}
        error={errors.date_of_presentation}
        required
...
const PaperPresentationStep2_Documents = ({
  formData,
  handleFileSelect,
  errors,
  setFormError,
...
const PaperPresentationStep3_Achievements = ({
  formData,
  handleChange,
  errors,
}) => (
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        ...
        <DetailItem
          label="Award or Special Recognition"
          value={formData.award}
        />
      </section>
    </div>
...
const STEP_CONFIG_PAPER_PRESENTATION = [
  ...
  {
    title: "Documents",
    validate: validateStep2_PaperPresentation,
    icon: Upload,
  },
  {
    title: "Achievements",
    validate: validateStep3_PaperPresentation,
    icon: Award,
  },
...
const PaperPresentation = ({ onBack, initialData = {} }) => {
  ...
  const validateCurrentStep = useCallback(() => {
    ...
    if (errors.pdf && !currentStepErrors.pdf)
      preservedFileErrors.pdf = errors.pdf;
    if (errors.certificate && !currentStepErrors.certificate)
      preservedFileErrors.certificate = errors.certificate;
  ...
  const nextStep = useCallback(() => {
    ...
    setCurrentStep((prev) =>
      Math.min(prev + 1, STEP_CONFIG_PAPER_PRESENTATION.length - 1)
    );
    window.scrollTo(0, 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (errors.pdf) {
      ...
      if (
        firstErrorStep === -1 ||
        firstErrorStep >
          STEP_CONFIG_PAPER_PRESENTATION.findIndex(
            (s) => s.title === "Documents"
          )
      ) {
        firstErrorStep = STEP_CONFIG_PAPER_PRESENTATION.findIndex(
          (s) => s.title === "Documents"
        );
      }
    }
    ...
    if (DEBUG_MODE) {
      console.log(
        `[DEBUG] Submitting paper presentation for rollno: ${rollno}`
      );
    }
    ...
    try {
      ...
      if (DEBUG_MODE)
        console.log("Paper Presentation submission successful:", response.data);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      case 0:
        return (
          <PaperPresentationStep1_Details
            formData={formData}
      ...
      case 1:
        return (
          <PaperPresentationStep2_Documents
            formData={formData}
      ...
      case 2:
        return (
          <PaperPresentationStep3_Achievements
            formData={formData}
            handleChange={handleChange}
            errors={errors}
          />
      ...
      case 3:
        return <PaperPresentationStep4_Review formData={formData} />;
      default:
        return null;
    }
  };
    ...
    return (
      <div className="max-w-3xl mx-auto bg-white py-10 px-4 sm:px-6 lg:px-8 mt-4 text-center rounded-lg shadow-xl">
        ...
        <h2 className="text-3xl font-bold text-gray-900 mb-3">
          Presentation Details Submitted!
        </h2>
        <p className="text-gray-600 mb-8 text-lg">
          Your paper presentation information has been successfully submitted
          for review.
        </p>
        {errors.submit && (
      ...
      <div className="flex items-center mb-8 border-b pb-5 border-gray-200">
        <div className="flex-grow">
          ...
          <p className="mt-1.5 text-sm text-gray-500">
            Fill in the details for your paper presentation.
          </p>
        </div>
      </div>
          ...
          <ol role="list" className="flex items-center">
            {STEP_CONFIG_PAPER_PRESENTATION.map((step, stepIdx) => (
              <li
                key={step.title}
                className={`relative ${
                  stepIdx !== STEP_CONFIG_PAPER_PRESENTATION.length - 1
