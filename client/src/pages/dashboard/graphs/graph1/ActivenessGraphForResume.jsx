...
const   ActivenessGraphForResume = () => {
  ...
  const handlePoints = async () => {
    try {
      ...
      console.error("Fetch error for activeness points:", error);
    }
  };
  ...
  const handleSemDays = async () => {
    try {
      ...
      console.error("Fetch error for semester days:", error);
    }
  };
  ...
  const { points, ticks } = useMemo(() => {
    ...
    (activenessData || []).forEach((item) => {
      if (!item || typeof item.sem === "undefined" || !item.currdate) return;
    ...
    Object.keys(semestersMap).sort().forEach((sem) => {
      const dateEntries = semestersMap[sem];
      const maxDays = semDayLimits[sem] || 75; // Fallback to 75
      ...
      if (semPoints.length) {
        ...
        if (semPoints[midIndex] && semPoints[midIndex].date instanceof Date && !isNaN(semPoints[midIndex].date)) {
          processedTicks.push({
    ...
    return { points: processedPoints, ticks: processedTicks };
  ...
  return (
    <div className="h-[250] w-full flex flex-col">
      <div className="mb-2">
      ...
      <div className="flex-grow h-[300px] w-full">
        {points.length > 0 ? (
          <Plot
            ...
            layout={{
              autosize: true,
              margin: { l: 30, r: 15, t: 5, b: 40 },
              paper_bgcolor: "rgba(0,0,0,0)",
              plot_bgcolor: "rgba(0,0,0,0)",
              dragmode: false, 
              xaxis: {
                type: "date",
                tickvals: ticks.map((t) => t.val),
