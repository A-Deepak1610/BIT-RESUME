...
export default function Faculty_Industry_ProjectsForm() {
    ...
    const handleFileChange = (e) => {
        if (e.target.files && e.target.files[0]) {
            setFormData((prev) => ({ ...prev, industryProjectProof: e.target.files[0] }));
    ...
    const handleDrop = (e) => {
        ...
        if (e.dataTransfer.files && e.dataTransfer.files[0]) {
            setFormData((prev) => ({ ...prev, industryProjectProof: e.dataTransfer.files[0] }));
            if (errors.industryProjectProof) {
                setErrors(prev => {
                    const newErrors = { ...prev };
                    delete newErrors.industryProjectProof;
    ...
    const handleSubmit = async (e) => {
        ...
        if (validate()) {
            try {
                ...
                const response = await axios.post(`${import.meta.env.VITE_API_URL}api/faculty/industryProjectPost`, data, {
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-8">
                        ...
                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2">Additional Faculty Members</h3>
                        <div>
                            <label htmlFor="numberOfFaculty" className="block text-sm font-medium text-gray-700 mb-1">No. of Faculty</label>
                            <input
                                type="number"
                        ...
                        {parseInt(formData.numberOfFaculty) > 0 && (
                            <div className="space-y-4">
                                {[2, 3, 4, 5].slice(0, parseInt(formData.numberOfFaculty)).map(num => (
                                    <div key={num} className="bg-gray-50 p-4 rounded-md border border-gray-200">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div>
                                                ...
                                                <input type="text" name={`faculty${num}`} value={formData[`faculty${num}`]} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Enter Name" />
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium text-gray-700 mb-1">Faculty {num} SIG</label>
                                                <input type="text" name={`faculty${num}SIG`} value={formData[`faculty${num}SIG`]} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Enter SIG" />
                                            </div>
                                        </div>
                                    </div>
                                ))}
                        ...
                        {/* Students Involved */}
                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2">Students Involved</h3>
                        <div>
                            <label htmlFor="numberOfStudents" className="block text-sm font-medium text-gray-700 mb-1">How many students are involved?</label>
                            <input
                                type="number"
                        ...
                        {parseInt(formData.numberOfStudents) > 0 && (
                            <div className="space-y-4">
                                {[1, 2, 3, 4, 5].slice(0, parseInt(formData.numberOfStudents)).map(num => (
                                    <div key={num} className="bg-blue-50 p-4 rounded-md border border-blue-200">
                                        <div>
                                            <label className="block text-sm font-medium text-gray-700 mb-1">Student {num}</label>
                                            <input type="text" name={`student${num}`} value={formData[`student${num}`]} onChange={handleChange} className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Enter Student Name" />
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ...
                        {/* Industry & Project Details */}
                        <h3 className="text-lg font-medium text-gray-900 border-b pb-2">Industry & Project Information</h3>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label htmlFor="industryProject" className="block text-sm font-medium text-gray-700 mb-1">
                                    Industry Project
                                </label>
                                <select
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">
                                ...
                                <span className="block text-xs text-gray-500 mt-1">(Approval Letter, Certificate, Project Report, Sample photos, Joint IPR)</span>
                            </label>
                            <div
                                className={`mt-1 flex flex-col items-center justify-center w-full h-32 px-6 pt-5 pb-6 border-2 ${errors.industryProjectProof ? 'border-red-500' : dragActive ? 'border-indigo-500 bg-indigo-50' : 'border-gray-300'
                                ...
                                <div className="space-y-1 text-center">
                                    ...
                                    <div className="flex text-sm text-gray-600">
                                        <label htmlFor="proof-upload" className="relative cursor-pointer rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                                            ...
                                            <input id="proof-upload" name="industryProjectProof" type="file" className="sr-only" onChange={handleFileChange} />
                                        </label>
                            ...
                            {formData.industryProjectProof && (
                                <div className="mt-2 flex items-center text-sm text-gray-600 bg-gray-50 p-2 rounded-md border border-gray-200">
                                    ...
                                    <span className="font-medium mr-2 truncate">{formData.industryProjectProof.name}</span>
                            ...
                            {errors.industryProjectProof && <p className="mt-1 text-sm text-red-600">{errors.industryProjectProof}</p>}
                        </div>
