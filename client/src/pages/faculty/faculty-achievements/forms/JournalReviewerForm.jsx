...
export default function JournalReviewerForm() {
  ...
  const validate = () => {
    ...
    if (
      ...
      newErrors.journalIndexing = "Journal Indexing is required";
    if (formData.journalIndexing === "Other" && !formData.otherJournalIndexing)
      newErrors.otherJournalIndexing = "Please specify";

    if (!formData.publisherName)
      newErrors.publisherName = "Publisher Name is required";
    if (!formData.journalHomepageURL)
      newErrors.journalHomepageURL = "Journal Homepage URL is required";
    if (
      !formData.recognitionType ||
      formData.recognitionType === "Click to choose"
    )
      newErrors.recognitionType = "Types of Recognition is required";
    if (formData.recognitionType === "Other" && !formData.otherRecognitionType)
      newErrors.otherRecognitionType = "Please specify";

    if (!formData.numberOfPapersReviewed)
      newErrors.numberOfPapersReviewed =
  ...
  const handleSubmit = async (e) => {
    ...
    if (validate()) {
      ...
      try {
        ...
        data.append("journalIndexing", formData.journalIndexing);
        if (formData.journalIndexing === "Other") {
          data.append("otherJournalIndexing", formData.otherJournalIndexing);
        }
        data.append("issnNo", formData.issnNo);
        data.append("publisherName", formData.publisherName);
        data.append("impactFactor", formData.impactFactor);
        data.append("journalHomepageURL", formData.journalHomepageURL);
        data.append("recognitionType", formData.recognitionType);
        if (formData.recognitionType === "Other") {
          data.append("otherRecognitionType", formData.otherRecognitionType);
        }
        data.append("numberOfPapersReviewed", formData.numberOfPapersReviewed);
        ...
        if (response.status === 200) {
          alert("Journal Reviewer record submitted successfully");
