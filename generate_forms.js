...
const forms = [
  { name: 'NewsletterForm', title: 'Newsletter / Department' },
  { name: 'EContentForm', title: 'E-Content' },
  { name: 'EventsAttendedForm', title: 'Events Attended' },
  { name: 'EventsOrganizedForm', title: 'Events Organized' },
  { name: 'ExternalExaminerForm', title: 'External Examiner' },
  { name: 'JournalReviewerForm', title: 'Journal Reviewer' },
  { name: 'GuestLectureForm', title: 'Guest Lectures' },
  { name: 'InternationalVisitForm', title: 'International Visits' },
  { name: 'AwardForm', title: 'Awards' },
  { name: 'OnlineCourseForm', title: 'Online Courses' },
  { name: 'PaperForm', title: 'Papers' },
  { name: 'ResourcePersonForm', title: 'Resource Person' },
];

const template = (componentName, title) => `import React, { useState } from "react";
...
export default function ${componentName}() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log("Submitting form for: ${title}", formData);
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="space-y-4">
              <div>
                ...
                >
                  Title / Name of Achievement <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  id="title"
...
const dir = 'client/src/pages/faculty/faculty-achievements/forms';
if (!fs.existsSync(dir)){
    fs.mkdirSync(dir, { recursive: true });
}
forms.forEach(form => {
  fs.writeFileSync(path.join(dir, form.name + '.jsx'), template(form.name, form.title));
